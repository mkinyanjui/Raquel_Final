...
def main():
    ...
    if df is not None:
        ...
        df_payroll_only = df_payroll_only.groupby(
            ['NAME_x', 'COMPANY CODE_x', 'POSITION ID_x', 'DEDUCTION CODE_x']).sum(['DEDUCTION AMOUNT']).reset_index()

        df_payroll_only.columns = [
            'NAME', 'COMPANY CODE', 'POSITION ID', 'DEDUCTION CODE', 'DEDUCTION AMOUNT', 'ENROLLMENT AMOUNT']
